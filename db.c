...
void db_init(){
    ...
    NULL,NULL,NULL);
    sqlite3_exec(db,"CREATE TABLE IF NOT EXISTS checker_state("
    "name TEXT PRIMARY KEY, enabled INTEGER);",NULL,NULL,NULL);
...
void db_save(const char *ip, int score,const char *country,const char *isp){
	sqlite3 *db;
	if (sqlite3_open("threats.db",&db) != SQLITE_OK){
		fprintf(stderr,"db open error: %s\n", sqlite3_errmsg(db));
		sqlite3_close(db);
		return;
	}
	sqlite3_stmt *stmt;
	if (sqlite3_prepare_v2(db,"INSERT INTO reports (ip,score,country,isp) VALUES (?,?,?,?);",-1,&stmt,NULL)!=SQLITE_OK){
		sqlite3_close(db);
		return;
	}
...
void db_save_setting(const char *key,const char *value){
	...
	sqlite3_stmt *stmt;
	if (sqlite3_prepare_v2(db,"INSERT OR REPLACE INTO settings (key,value) VALUES (?,?);",-1,&stmt,NULL)!=SQLITE_OK){
		sqlite3_close(db);
		return;
	}
...
void db_cache_save(const char *ip, const char *checker, const char *result){
	...
	sqlite3_stmt *stmt;
	if (sqlite3_prepare_v2(db,"INSERT OR REPLACE INTO cache(ip,checker,result) VALUES(?,?,?);",-1,&stmt,NULL)!=SQLITE_OK){
		sqlite3_close(db);
		return;
	}
...
char *db_cache_get(const char *ip, const char *checker){
	...
	if (sqlite3_prepare_v2(db,"SELECT result FROM cache WHERE ip=? AND checker=? AND checked_at>datetime('now','-24 hours');",-1,&stmt,NULL)!=SQLITE_OK){
		sqlite3_close(db);
		return NULL;
	}
	sqlite3_bind_text(stmt,1,ip,-1,SQLITE_STATIC);
...
void db_save_checker_state(const char *name,int enabled){
	...
	sqlite3_stmt *stmt;
	if (sqlite3_prepare_v2(db,"INSERT OR REPLACE INTO checker_state(name,enabled) VALUES(?,?);",-1,&stmt,NULL)!=SQLITE_OK){
		sqlite3_close(db);
		return;
	}
...
int db_get_checker_state(const char *name){
	...
	sqlite3_stmt *stmt;
	if (sqlite3_prepare_v2(db,"SELECT enabled FROM checker_state WHERE name=?;",-1,&stmt,NULL)!=SQLITE_OK){
		sqlite3_close(db);
		return 1;
	}
	sqlite3_bind_text(stmt,1,name,-1,SQLITE_TRANSIENT);
